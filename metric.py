...
class Metrics:
    def __init__(self, metrics_of_interest=["mae", "iou", "sa_sor", "sor"]):
    ...
    def process(self, preds, gts, thres=.5, **kwargs):
        """
    ...
    def jacc(self, gt_ind, pd_ind, pred_keypoints, gt_keypoints, **argw):
        gt = gt_keypoints[gt_ind].numpy()
        pred = np.stack([pred_keypoints[i] for i in pd_ind])
        label = (gt.reshape(-1, 3)[:, -1] > 0.5).astype(float)
        ispose = (pred.reshape(-1, 3)[:, -1] > 0.5).astype(float)
        precision = ((label * ispose).sum() + 1e-6) / (label.sum() + 1e-6)  ## precision
        if label.sum() > 0.5:
            return precision
        else:
            return None

    def jae(self, gt_ind, pd_ind, pred_keypoints, gt_keypoints, **argw):
        gt = gt_keypoints[gt_ind].numpy()
        pred = np.stack([pred_keypoints[i] for i in pd_ind])
        label = (gt.reshape(-1, 3)[:, -1] > 0.5).astype(float)
        err = np.sum(np.abs(gt.reshape(-1, 3)[:, 0:2] - pred.reshape(-1, 3)[:, 0:2]) * label.reshape(-1, 1)) / (
                    1e-6 + label.sum())
        if label.sum() > 0.5:
            return err
        else:
...
if __name__=="__main__":
    ...
    for name in tqdm.tqdm(names): #[0:100]
        ...
        gts = decompose(np.array(Image.open(os.path.join(gt, name)).convert("L"), dtype=np.uint8))
        # results.append(metrics.process(preds, gts, thres=.5))
        results.append(metrics.process(preds, gts, thres=args.threshold))
    report = metrics.aggregate(results)
    
    print(report, '\n')

    print(input_path)
    print([f"{key}: {value}" for key, value in report.items() if key in ['mae', 'sa_sor', 'sor']])

    save_info_path = input_path.replace('final-gray', '').replace('final_gray', '')
    print('\nSaving metric results to ', save_info_path)
    with open(save_info_path + '/metrics.txt', 'a') as f:
        f.write(f'\n{input_path}\n' + str([f"{key}: {value}" for key, value in report.items() if key in ['mae', 'sa_sor', 'sor']]) + '\n')
