...
from .modeling.language_encoder.bert import BertEncoder
...
class MaskFormer(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.lang_encoder = BertEncoder('bert-base-uncased', 
                                        use_checkpoint=True, 
    ...
    def forward(self, batched_inputs):
        ...
        # pos_l = [x['phrases']['pos_l'].to(self.device) for x in batched_inputs]
        # pos_r = [x['phrases']['pos_r'].to(self.device) for x in batched_inputs]
        text_info = {'tokens': {
                    'input_ids': torch.stack(input_ids).squeeze(1),
                    'attention_mask': torch.stack(attention_mask).squeeze(1)}}
        
        # memory_text = lan_out['embedded']
        # if self.add_pooling_layer:
        #     memory_text = lan_out['pooler_output'].unsqueeze(1)
        ...
        phrase_out = self.lang_encoder({'input_ids': p_input_ids, 'attention_mask': p_attention_mask})
        ...
        if 'relations' in batched_inputs[0]:
            ...
            relation_out = self.lang_encoder({'input_ids': relation_tokens.view(bs*n_relations, -1),
